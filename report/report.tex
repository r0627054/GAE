...
(a) Is there a scenario in which the code to confirm the quotes is executed multiple times in parallel,
resulting in a positive confirmation to both clients’ quotes?\mbox{}\\\\

(b) If so, can you name and illustrate one (or more) possibilities to prevent this bogus behaviour?\mbox{}\\\\

(c) In case your solution to the previous question limits parallelism, would a different design of
...
assume that a client will have quotes belonging to one car rental company only.\mbox{}\\\\
	...
	\ref{fig:structure}
	Since the only type of relationship between two entities is a one-to-many (child/parent) relation, we created a parent root entity for each CarRentalCompany. Each company has a list of CarTypes, which have a list of Cars. And these cars have a list of reservations, since we never store quotes permanently.
